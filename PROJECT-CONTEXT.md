...
The overwhelming majority of Amazon product links (230 of 234, verified) use **`amzn.to`**
shortened links (Amazon's own link shortener, with the affiliate tag embedded server-side) —
this is the established, correct pattern; don't "fix" these into full URLs. **Correction to an
earlier version of this document:** this is not universal — `outfits.html` specifically has 4
links written as full `https://www.amazon.com/dp/{ASIN}?tag=thepixiepacke-20` URLs instead (2
...
- **Stale footer label:** `outfits.html` was renamed from "Themed Outfit Inspiration" to "Shop
  ...
  pointing to it on every OTHER page still says **"Themed Outfits"** — verified present on all
  19 other pages in the repo. The link itself still works (correct `href="outfits.html"`), this
  is a label-only inconsistency, not a broken link. Low priority, but a quick fix once someone's
  editing footers anyway.
