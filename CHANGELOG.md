[CHANGELOG.md](https://github.com/user-attachments/files/30881909/CHANGELOG.md)
[CHANGELOG.md](https://github.com/user-attachments/files/30876312/CHANGELOG.md)
...
- This was caught and reverted the same day — commit message: *"Update print statement from
  ...
  **Full root cause, reconstructed from an earlier project conversation [from conversation]:**
  ...
  3. The damaging commit was initially hard to find: the first pass of investigation only
     ...
     `index.html` specifically and checking every entry, not just # CHANGELOG.md — The Pixie Packed Family
...
- **CMS upgrade:** local prototype only, Checkpoint 3 **closed** — see "CMS Upgrade — Checkpoint
  History" above. Production is unaffected; Checkpoint 4 has not begun and requires Vanessa's
  separate explicit approval to start.
...
## CMS Upgrade — Checkpoint History

**[from conversation]** The next major project for this site is to **separate website content
from website code** by building a secure, browser-based admin/content-management system, so
Vanessa can manage products, images, blog posts, guides, and affiliate links directly, without
manually editing files in GitHub or waiting on a code change and redeploy for every update. This
is a significant architectural shift from the current all-static, no-backend setup described
elsewhere in this document. Chosen approach: Eleventy (static generation) + Sveltia CMS
(git-based content editor) + GitHub Actions (build/deploy, kept separate from Remy's Railway
environment). Full data model and technical detail lives in ARCHITECTURE.md's "CMS Content
Architecture" section — this section is the chronological/status record.

**Current status: local prototype only, Checkpoint 3 closed, not connected to production.**
Both issues that kept Checkpoint 3 open — the Category↔Look relationship-sync bug and the
Product Library thumbnail issue — are resolved and human-verified. See ARCHITECTURE.md for full
technical detail on both. Checkpoint 4 (production connection) has not begun and requires
Vanessa's separate, explicit approval to start.

- **Checkpoint 0 — Recovery point.** Before any implementation, the exact then-current production
  commit hash was captured and recorded as the rollback target. Approach confirmed: because
  `main` is branch-protected and every change goes through a PR, rollback never means a force-push
  or history rewrite — it's always one more ordinary revert PR, reviewed and merged the same way
  as any other change.
- **Checkpoint 1 — Eleventy proof of concept.** Built the minimum Eleventy setup to regenerate
  `accessories-backpacks.html` from structured Markdown/YAML content instead of hand-authored
  HTML. Result: **byte-for-byte identical** output to the real production file, verified by
  direct diff and matching SHA-256 checksums. Proved the static-generation approach can
  reproduce the existing site exactly before any visual/structural risk is introduced.
- **Checkpoint 2 — Sveltia CMS local prototype.** Built a local, isolated Sveltia CMS
  configuration (no production GitHub credentials, no OAuth, standalone git repo with no
  remote) scoped to managing Backpacks. Vanessa's first hands-on usability test: could log in
  via "Work with Local Repository," see/open/edit the 3 real existing products, and the
  workflow made sense without needing to understand GitHub, Markdown, or Git concepts.
- **Checkpoint 3 — Relational content architecture, closed.** Replaced the flat, Backpacks-only
  content model with the full Products/Collections/Categories/Looks relational model described
  in ARCHITECTURE.md. Re-verified `accessories-backpacks.html` still generates byte-identical to
  production under the new model. Built and tested a real second content type end-to-end — the
  "Minnie in Pink" Look, using its actual real content pulled from the live `outfits.html` (8
  real products, real collage image, real Amazon links) — including master-product-reuse,
  replace-in-look, hidden-product-inside-published-look, and stable-URL-survives-rename-and-move
  tests, all verified working. Self-service Collection/Category creation was proven at the
  build level. Human usability test results: creating a Collection and adding a Product both
  **PASS, easy**; overall CMS usability **PASS**.

  Two issues kept this checkpoint open for an extended investigation; both are now resolved:
  - **Product Library thumbnails** — root cause was two compounding problems: (1) a
    Netlify/Decap-era relative `media_folder`/`public_folder` convention that didn't match
    Sveltia's current documented requirements, corrected to `media_folder: "/src/photos"` /
    `public_folder: "/photos"`, with existing Product image paths migrated to match; (2)
    `type="module"` on Sveltia's script tag, which Sveltia's own current docs flag as
    unnecessary and a source of "unexpected behavior" — removing it resolved a separate
    new-Product finalize bug (orphaned `.sveltia-tmp-*` files). Both fixes were isolated and
    confirmed independently before being combined. Final confirmed-working image workflows:
    existing/migrated images, Amazon-copy-paste (Vanessa's real workflow), local file
    drag-and-drop, no-image Products, and stock photo selection all **PASS**. Dragging an image
    directly off a webpage into Sveltia does **not** work and is explicitly not a required
    workflow — deferred, not a blocker.
  - **Category↔Look relationship sync** — redesigned so the Look is the single source of truth
    for its own Category membership; Category pages derive their Look list at build time by
    filtering, never from a stored list. `look_items` removed from the schema entirely. Moving a
    Look between Categories (a one-field edit) was verified to leave no stale references, and the
    Look's URL was confirmed unchanged by the move. Ordering within a Category uses a "Display
    Position" number field with Sveltia's `sortable_fields` default-sort (not `reorder`/
    `view_groups`, which didn't work reliably in browser testing) — verified: the list opens
    pre-sorted, updates live when a position changes, and the Category-search-then-re-sort
    workflow reliably produces a "this Category, in real order" view. Adopted numbering
    convention: spaced integers (10, 20, 30…), not decimals — see ARCHITECTURE.md.

  **Deferred, non-blocking, explicitly not reopened by future sessions:** dragging images
  directly from a webpage into Sveltia (unsupported, not needed); the Type A (Product-only)
  Category latent structural risk noted in ARCHITECTURE.md (nothing has broken, not in scope);
  Sveltia's version remains intentionally unpinned (confirmed `0.183.0` during this checkpoint's
  testing, may drift).

## DO NOT REPEAT — CMS/Sveltia-specific additions

- **Chrome's File System Access API can silently reuse a previously-granted local folder handle
  at the same `localhost` origin across different CMS test packages.** This caused a real,
  time-consuming debugging cycle during Checkpoint 3: a new, correctly-built local package
  appeared broken (existing content invisible in the CMS) purely because the browser was still
  reading an old package's folder from a prior test at the same port. The `config.yml` itself
  loading correctly is not evidence the right folder is being read — those are two independent
  things. **Always use a fresh port for each new local test package.**
- **When a documented, correctly-applied configuration fix doesn't resolve an observed bug,
  don't add a second workaround on top of it — stop and investigate the actual software
  behavior/version first.** This is the standing rule after the Product Library thumbnail issue
  persisted through an explicit, docs-correct `thumbnail: image` fix.
- **Always verify a claimed documentation source directly (fetch the actual primary-source
  page) rather than relying on a search-result snippet or a differently-sourced page that
  merely looks related.** A citation error occurred once during this project — content that was
  actually from Sveltia's own docs got attributed with the wrong source link, which was caught
  and corrected by re-fetching the real page directly. Treat this as the standing bar for any
  future "verify against current documentation" task in this project.
- **Don't carry over Netlify/Decap CMS media-path conventions into Sveltia without checking
  current docs.** The original `media_folder: "../photos"` / `public_folder: "photos"` config
  followed the older, config-file-relative convention; Sveltia's current docs require top-level
  `media_folder` to be repo-root-relative and `public_folder` to have a leading slash. This
  mismatch was the root cause of the Product Library thumbnail issue (see ARCHITECTURE.md).
  Correct values for this project: `media_folder: "/src/photos"`, `public_folder: "/photos"` —
  and any new Product/Look image path must use the leading-slash form.
- **Do not set `type="module"` on Sveltia's script tag.** Sveltia's `.js` CDN build is not an ES
  module; this attribute caused real, reproducible new-entry finalize failures (orphaned
  `.sveltia-tmp-*` files on clipboard-paste image saves). Confirmed fixed by removing it.
- **Don't rely on Sveltia's `reorder`/`view_groups` collection options for entry ordering/
  grouping.** Configured exactly per current documentation, neither produced the documented
  behavior in real browser testing (no visible grouping, no working drag control). Use
  `sortable_fields` with a `default` sort instead — a pre-set list sort, not drag-and-drop, and
  confirmed working. If reconsidering `reorder`/`view_groups` in the future, treat it as
  unverified, not as "should work because the docs say so."
- **The Sveltia CDN URL used here is intentionally unversioned** (resolves to whatever is
  current "latest"; confirmed `0.183.0` during Checkpoint 3). This is expected/documented
  behavior, not a bug — but it means the exact version in use can drift between sessions. If
  behavior ever seems inconsistent with what's recorded here, check the actual resolved version
  before assuming the config regressed.
the latest diff.
  4. This was investigated at the time and confirmed to **NOT** be a DNS problem, a GitHub Pages
     misconfiguration, or a second repository competing for the domain — all of those were
     checked directly and ruled out.
...
- **Monetization live:** Amazon affiliate links (`amzn.to`) across all product pages; one paid
  digital product ($4 Disney Vacation Planning Guide) via Gumroad
- **CMS upgrade:** local prototype only, through Checkpoint 3 — see "CMS Upgrade — Checkpoint
  History" above. Production is unaffected; two issues remain unresolved before Checkpoint 4.
...
**Current status: local prototype only, through Checkpoint 3, not connected to production.**
Two issues are unresolved and must be addressed, with Vanessa's approval, before Checkpoint 4:
the Category↔Look relationship-sync bug, and the Product Library thumbnail issue not showing
saved images. See ARCHITECTURE.md for both in full detail.
...
- **Checkpoint 2 — Sveltia CMS local prototype.** Built a local, isolated Sveltia CMS
  ...
  via "Work with Local Repository," see/open/edit the 3 real existing products, and the
  workflow made sense without needing to understand GitHub, Markdown, or Git concepts.
- **Checkpoint 3 — Relational content architecture.** Replaced the flat, Backpacks-only content
  model with the full Products/Collections/Categories/Looks relational model described in
  ARCHITECTURE.md. Re-verified `accessories-backpacks.html` still generates byte-identical to
  production under the new model. Built and tested a real second content type end-to-end — the
  "Minnie in Pink" Look, using its actual real content pulled from the live `outfits.html` (8
  real products, real collage image, real Amazon links) — including master-product-reuse,
  replace-in-look, hidden-product-inside-published-look, and stable-URL-survives-rename-and-move
  tests, all verified working. Self-service Collection/Category creation was proven at the
  build level (a new Collection + Category, created as plain files with zero code/template
  changes, generated correct pages automatically; deleting them removed the generated pages
  cleanly). Human usability test results: creating a Collection and adding a Product both
  **PASS, easy**; overall CMS usability **PASS**; visual identification of saved Product
  entries **NEEDS IMPROVEMENT** (see the thumbnail issue below). Two issues remain open — see
  ARCHITECTURE.md for the Category↔Look sync bug and the Product Library thumbnail issue, the
  latter of which is still unresolved even after an explicit, correctly-applied
  `thumbnail: image` config fix.
